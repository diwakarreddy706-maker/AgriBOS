...
import { Plus, Search, Gauge, Trash2, MapPin, CheckCircle2, AlertTriangle, AlertCircle, Tractor } from 'lucide-react';
...
export const TractorListPage: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['tractors', search, page],
    queryFn: () => machineApi.getMachines(search, 'TRACTOR', undefined, page, 10),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['tractors'] }),
  });
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
              {t.tractors || 'Tractors & Heavy Machinery'}
            </h1>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Dedicated Tractor fleet management, RTO compliance, engine hours, driver assignment & transport work
          </p>
        </div>
        <Button onClick={() => setIsDialogOpen(true)} className="flex items-center space-x-2 bg-blue-600 hover:bg-blue-700">
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            <div className="p-8 text-center text-xs text-slate-500">Loading tractors...</div>
          ) : !data?.content || data.content.length === 0 ? (
            <div className="p-8 text-center text-xs text-slate-500">No tractor records found.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-100 dark:bg-slate-800/60 text-slate-600 dark:text-slate-400 font-semibold border-b border-slate-200 dark:border-slate-800">
                  <tr>
                    <th className="p-3">Tractor Code</th>
                    <th className="p-3">Model & RTO Reg</th>
                    ...
                    return (
                      <tr key={m.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                        ...
                        <td className="p-3">
                          ...
                          <div className="text-[10px] font-mono text-blue-600 font-bold">{m.registrationNumber || 'RTO Pending'}</div>
                        </td>
                        <td className="p-3">
                        ...
                        <td className="p-3">
                          ...
                          {(status === 'SERVICE_DUE' || status === 'OVERDUE') && (
                            <button
                              ...
                              className="ml-2 text-[10px] font-bold text-blue-600 underline hover:text-blue-800"
                            >
                              Complete Service
                            </button>
                          )}
                        ...
                        <td className="p-3 font-mono">
                          <div>{formatCurrency(m.hourlyRateDefault)}/hr</div>
                          <div className="text-[10px] text-slate-400">{formatCurrency(m.acreRateDefault)}/acre</div>
                        </td>
                        <td className="p-3">
                          <span
                        ...
                        <td className="p-3 text-right">
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              if (confirm(`Are you sure you want to delete tractor ${m.machineCode}?`)) {
      ...
      <MachineFormDialog
        ...
        onSubmit={async (formData) => {
          await createMutation.mutateAsync({ ...formData, machineType: formData.machineType || 'TRACTOR' });
        }}
